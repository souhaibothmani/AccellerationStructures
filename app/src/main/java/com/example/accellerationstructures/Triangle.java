...
public class Triangle implements Hittable {
    ...
    public Intersection intersect(Ray ray, int objectIndex) {
        ...
        //Step 1 - reuse Moller-Trumbore math
        //already handles parallel ray , outside triangle , behind origin
        //returns [t , u , v] or null if miss
        float[] result = intersect(ray);

        //Step 2 - missed
        if(result == null) return null;

        //Step 3 - extract t, u, v from result
        float t = result[0];
        float u = result[1];
        float v = result[2];
        ...
        //Step 5 - interpolate normal using barycentric coordinates
        //u, v come directly for the step 1 (moller-trumbore)
        Vec3 normal = getInterpolatedNormal(u , v);
